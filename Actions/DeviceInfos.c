...
void showDeviceInfos() {
    ...
    sprintf(deviceInfosCommand, "adb shell getprop > SRLinker_DeviceInfos/%s_deviceInfos.txt", timestamp);
    FILE* adbOutputSave;
    ...
    for (int i = 0; i < sizeof(deviceInfos1) / sizeof(deviceInfos1[0]); i++) {
        sprintf(deviceInfosCommand, "adb shell getprop %s >> SRLinker_DeviceInfos/%s_deviceInfos.txt", deviceInfos1[i], timestamp);
    ...
    for (int i = 0; i < sizeof(deviceInfos) / sizeof(deviceInfos[0]); i++) {
        sprintf(adbDevicesCommand, "adb shell getprop %s", deviceInfos[i]);
